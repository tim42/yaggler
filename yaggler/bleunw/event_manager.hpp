...
// in : file:///home/tim/projects/yaggler/yaggler/bleunw/event_manager.hpp
//
...
namespace neam
{
  namespace bleunw
  {
    namespace yaggler
    {
      ...
      {
        public:
          event_manager(neam::yaggler::glfw_window &_win) : win(_win)
          {
            // register glfw callbacks and user pointer
            glfwSetWindowUserPointer(win._get_glfw_handle(), this);

            glfwSetKeyCallback(win._get_glfw_handle(), &event_manager::_t_key);
            glfwSetCharCallback(win._get_glfw_handle(), &event_manager::_t_unicode_input);
            glfwSetMouseButtonCallback(win._get_glfw_handle(), &event_manager::_t_mouse_button);
            glfwSetCursorPosCallback(win._get_glfw_handle(), &event_manager::_t_mouse_move);
            glfwSetScrollCallback(win._get_glfw_handle(), &event_manager::_t_mouse_wheel);
        ...
        private: // trampos
          ...
          {
            event_manager *emgr = reinterpret_cast<event_manager *>(glfwGetWindowUserPointer(glfw_win));
